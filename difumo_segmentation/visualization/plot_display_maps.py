...
def plot_atlases_image(root_dir, percent=0.33, cmap=matplotlib.colors.ListedColormap('k', name='from_list', N=256)):
    ...
    atlas_img_name = "tpl-MNI152NLin2009cAsym_res-0{res}_atlas-DiFuMo_desc-{dim}dimensionsSegmented_probseg.nii.gz"
    atlas_label_name = "tpl-MNI152NLin2009cAsym_res-0{res}_atlas-DiFuMo_desc-{dim}dimensionsSegmented_probseg.tsv"

    for i, n_components in enumerate(components_to_display):
        for res in resolutions_to_display:
            maps_img = os.path.join(root_dir, "data", "processed", "segmented_difumo_atlases",
                                    "tpl-MNI152NLin2009cAsym", atlas_img_name.format(dim=n_components, res=res))
            label_path = os.path.join(root_dir, "data", "processed", "segmented_difumo_atlases",
